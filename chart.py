...
class Chart:
    ...
    def read_chart(self, filename):
        mid = MidiFile(filename)
        gtr_indices = [i for i in range(len(mid.tracks)) if mid.tracks[i].name == "PART GUITAR"]
        track = mid.tracks[gtr_indices[0]]
        ...
        for msg in track:
            ...
            if msg.type not in ['note_on', 'note_off']:
                pass
            elif (msg.note in mid_to_mod.keys()):
                if msg.type == 'note_on':
                    active_mods.append(msg.note)
                elif msg.type == 'note_off':
                    active_mods.remove(msg.note)
            elif msg.note not in mid_to_note.keys():
                pass
